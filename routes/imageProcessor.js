...
const processFiles = async (files, userId, postId, Media) => {
  ...
  const mediaEntries = []; // Array to store media entries for batch insertion

  // Use Promise.all to process images in parallel
  await Promise.all(
    files.map(async (file) => {
      ...
      // Store media entry for batch insertion later
      mediaEntries.push({
  ...
  // Insert all media entries in one go if possible
