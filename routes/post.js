...
const createPost = async (req, res) => {
  ...
  activityLocations.forEach(location => {
    const distance = calculateDistance(userProvidedLocation[0], userProvidedLocation[1], location.latitude, location.longitude);
    if (distance < closestDistance) {
      closestDistance = distance;
    }
  });

  // Set a threshold for distance verification, e.g., 50 meters
  const distanceThreshold = 0.05; // 50 meters in degrees
  const verificationPercentage = (closestDistance <= distanceThreshold) ? 100 : 0; // 100% if within threshold, else 0
